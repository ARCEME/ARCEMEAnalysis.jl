...
function hrl_warp(cubename, fnames::Vector; batch="ARCEME-DC-6")
  ...
  for hrl in keys(hrl_dict)
    ...
    props = Dict(
        ...
        "resolution" => 10,
        "unit" => "meter",
        "source" => "$(hrl_fnames[1]) © European Union, Copernicus Land Monitoring Service 2025, European Environment Agency (EEA)",
        "processing" => "Downloaded from wEkEO. Reprojected to EPSG $(ds.properties["epsg"]) using nearest neighour and clipped to AOI with gdalwarp",
