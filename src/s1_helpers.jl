...
"""
function arceme_s1_position(ds; distance_threshold=61)
    ...
    for its in 2:length(ts)
        ...
        for igroup in eachindex(groups)
            ...
            if abs(round(fac) - fac) < (distance_threshold * 1000 / sixdays)
        ...
        if !groupfound
            newoffset = mod(Millisecond(ts[its] - ts[1]).value, sixdays) #- floor(Int, fac) * sixdays
